...
//审核按钮
function auditConfig(url, parm) {
    showConfirmMsg('注：您确认要审核这些信息吗？', function (r) {
        if (r) {
            getAjaxJason(url, parm, function (rs) {
                if (!rs) {
                    ...
                    showTipsMsg("审核失败，0 行受影响！", 3000, 3);
                }
                else {
...
//审核退回按钮
function auditReturnConfig(url, parm) {
    showConfirmMsg('注：您确认审核退回这些信息吗？', function (r) {
        if (r) {
            getAjaxJason(url, parm, function (rs) {
                if (!rs) {
                    showTipsMsg("审核退回成功！", 2000, 4);
                    windowload();
                } else if (length(rs) == 0) {
                    showTipsMsg("审核退回失败，0 行受影响！", 3000, 3);
                }
                else {
                    showTipsMsg("<span style='color:red'>审核退回失败，请稍后重试！</span>", 4000, 5);
                }
            });
        }
    });
