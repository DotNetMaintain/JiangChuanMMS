...
namespace RM.Web.MMS.MMS_Requisition
{
    ...
    private DataTable  findDeptManage()
    {
       ...
       return dt_DeptInfo;
        ...
        private string FindAuditUser(List<TPurchasePlanDetail> lstproductcode)
        {
            string AuditUser = string.Empty;
			string lstmaterial = string.Empty;
            int i = 0;
            foreach (TPurchasePlanDetail tp in lstproductcode)
            {
...
group by productcode,price) purplan)purplan
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Page.IsValid)
        {
            ...
            if (string.IsNullOrEmpty(id)) //插入
            {
                ...
                info.Content.PurchaseMan = FindAuditUser(info.Detail).Trim();
                info.Content.PayMode = "1";
                info.Content.InvoiceType = findDeptManage().Rows[0]["Organization_Address"].ToString().Trim();
                // info.Content.DeptName = findDeptManage().Rows[0]["Organization_Name"].ToString().Trim();
            ...
            if (info.Detail.Count > 0)
            {
                ...
                foreach (TPurchasePlanDetail d in info.Detail)
                {
                    ...
                    if (total < detailqua)
                    {
                            ...
                            Response.Write("<Script>window.alert('物料编码为" + d.DetDetail.ProductCode + "的物料，您的领料数量：" + detailqua + "大于库存：" + total + "！')</Script>");
    ...
    private void ModelToTextBox(TPurchasePlan info)
    {
        ...
        txtDeptName.Text = info.Content.DeptName;
