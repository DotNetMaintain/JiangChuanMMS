...
namespace RM.Busines.DAL
{
    public class RM_UserInfo_Dal : RM_UserInfo_IDAO
    {
        ...
        public string GetUserInfoName(string Account)
        {
            string re = "";
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT *  FROM Base_UserInfo WHERE User_Account = @Account");
            SqlParam[] para = new SqlParam[]
            {
                new SqlParam("@Account", Account)
            };
            DataTable dt=DataFactory.SqlDataBase().GetDataTableBySQL(strSql, para);
            if (dt.Rows.Count > 0 && dt != null)
            {
                re = dt.Rows[0]["User_Name"].ToString();
            }
            return re;
        }
